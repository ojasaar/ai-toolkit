    ...
    def setup_buckets(self: 'AiToolkitDataset', quiet=False):
        ...
        for idx, file_item in enumerate(file_list):
            ...
            if self.dataset_config.square_crop:
                ...
                # Aspect-ratio-preserving resize (no cropping)
                # This bypasses the old fixed SDXL bucket system
                # Calculate target dimensions based on max pixels while preserving aspect ratio
                max_pixels = resolution * resolution  # e.g., 1024 * 1024 = 1,048,576
                aspect_ratio = width / height

                # Use same logic as diffusers calculate_dimensions
                new_width = math.sqrt(max_pixels * aspect_ratio)
                new_height = new_width / aspect_ratio

                # Round to multiples of bucket_tolerance (typically 32)
                new_width = round(new_width / bucket_tolerance) * bucket_tolerance
                new_height = round(new_height / bucket_tolerance) * bucket_tolerance

                # No cropping - scale_to dimensions match crop dimensions
                file_item.scale_to_width = int(new_width)
                file_item.scale_to_height = int(new_height)
                file_item.crop_width = int(new_width)
                file_item.crop_height = int(new_height)
                file_item.crop_x = 0
                file_item.crop_y = 0

                # Log the aspect-ratio-preserving resize (first 3 images only to avoid spam)
                if idx < 3:
                    print_acc(f"[BUCKET] Image {idx}: {width}x{height} (ratio {aspect_ratio:.3f}) -> {int(new_width)}x{int(new_height)} (no crop, aspect preserved)")
        ...
        if not quiet:
            print_acc(f'[BUCKET] Using aspect-ratio-preserving bucketing (NO fixed SDXL buckets, NO cropping)')
            print_acc(f'Bucket sizes for {self.dataset_path}:')
            for key, bucket in self.buckets.items():
                print_acc(f'{key}: {len(bucket.file_list_idx)} files')
            print_acc(f'{len(self.buckets)} buckets made (each preserves original aspect ratio)')
